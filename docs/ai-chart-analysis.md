...
- SMA(5, 10, 20, 50): 3개월 안의 단기·중기 배열을 확인한다.
...
2. 3개월(60일), 중기(50/20일), 단기(10/5일) 국면을 분리한다.
...
6. 각 날짜의 예상 종가, 예상 범위, 신뢰도와 함께 현재 주문 가이드상 예상되는 매수·매도·관망·모드 전환 가능성을 반환한다.
...
- 표시 항목: 한국시간 분석 시각, 핵심 결과, 짧은 판단 근거, 5거래일 예상 종가·가격 범위·매매 흐름, 핵심 예측 변수.
- 상세 지표와 장문의 분석 설명은 화면에서 제외하고 가격 및 매매 결과를 우선한다.
